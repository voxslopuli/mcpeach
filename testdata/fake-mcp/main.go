...
func main() {
	...
	// the PID, args, and selected env vars so tests can verify they reach the
	// subprocess and track its lifecycle without platform-specific tools.
	fmt.Fprintf(os.Stderr, `{"level":"info","msg":"fake-mcp started","pid":%d,"args":%q,"env":{"MC_TEST_VAR":%q,"MC_CONFIG_VAR":%q},"time":%q}`+"\n", os.Getpid(), os.Args[1:], os.Getenv("MC_TEST_VAR"), os.Getenv("MC_CONFIG_VAR"), time.Now().Format(time.RFC3339))
