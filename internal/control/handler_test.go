...
func TestConcurrentControlOps(t *testing.T) {
	...
	for i := 0; i < 20; i++ {
		...
		go func(i int) {
			...
			rec := httptest.NewRecorder()
			h.ServeHTTP(rec, req)
			req = httptest.NewRequest(http.MethodPost, "/v0/servers/"+name+"/start", nil)
			rec = httptest.NewRecorder()
			h.ServeHTTP(rec, req)
			req = httptest.NewRequest(http.MethodPost, "/v0/servers/"+name+"/stop", nil)
			rec = httptest.NewRecorder()
			h.ServeHTTP(rec, req)
		}(i)
	}
